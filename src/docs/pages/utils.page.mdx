...
export const FlattenedList = () => {
  ...
  const flatList = flattenNestedList(nestedList, 'children');

  return (
    <ul>
      {flatList.map(item => (
        <li key={item.id} style={{ marginLeft: item.depthLevel * theme.spacings.base * 2 }}>
          {item.name} (Parent Index: {item.parentIndex ?? '-'})
        </li>
      ))}
    </ul>
  )
...
The `flattenNestedList` function converts a nested list structure into a flat list, excluding the nested list key from the result and adding both a `depthLevel` and `parentIndex` property to each item. This is useful for displaying hierarchical data in a flat structure while maintaining information about the hierarchy.

### Parameters

- `items` \{Item[] | undefined\}: The array of items to be flattened. This array can be undefined.
- `nestedListKey` \{KeysWithArrayValues\<Item\>\}: The key in each item that contains the nested list.
- `result` \{HoneyFlattenedItem\<Item, typeof nestedListKey\>[]\} [default=[]\]: The array that accumulates the flattened items. Defaults to an empty array.
...
A flat array of items, excluding the nested list key and including both a `depthLevel` and `parentIndex` property.

### Usage

Flattens a nested list structure to create a flat list with depth levels and parent indices, making it easier to display hierarchical data in components like lists or trees.
